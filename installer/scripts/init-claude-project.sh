...
print_next_steps() {
    ...
    echo ""
    echo -e "${BOLD}require-kit Workflow:${NC}"
    echo ""
    ...
    echo ""
    echo "Stage 2: Feature & Task Specification"
    echo -e "   ${BLUE}/feature-create${NC}      - Create feature with epic linkage"
    echo -e "   ${BLUE}/generate-bdd${NC}        - Create test scenarios"
    echo -e "   ${BLUE}/feature-generate-tasks${NC} - Generate task specifications"
    echo ""
    echo "Stage 3: Hierarchy & Export"
    echo -e "   ${BLUE}/hierarchy-view${NC}      - View project hierarchy"
    echo -e "   ${BLUE}/epic-sync${NC}           - Sync to PM tools (Jira, Linear, etc.)"
    echo ""

    # Check if taskwright is installed to show task execution commands
    if [ -f "$HOME/.agentecflow/taskwright.marker" ]; then
        echo -e "${BOLD}taskwright Integration (Detected):${NC}"
        echo ""
        ...
        echo -e "${YELLOW}Task Execution (requires taskwright):${NC}"
        echo "  Install taskwright for task execution workflow:"
        echo "  https://github.com/taskwright-dev/taskwright"
        echo ""
    fi
