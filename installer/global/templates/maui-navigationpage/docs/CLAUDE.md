...
private async Task SelectProduct()
{
    ...
    await Navigator.NavigateToAsync<ProductSelectionViewModel>();
...
4. **Always inject INavigator into ViewModels**:
   ...
   {
       public ProductDetailViewModel(INavigator navigationService)
...
```csharp
[Fact]
public async Task NavigateToProductDetail_ShouldCallNavigator()
{
    // Arrange
...
public class PersonalInfoViewModel : ViewModelBase<WizardParams>
{
    ...
    private async Task Next()
    {
        ...
        await Navigator.NavigateToAsync<AddressInfoViewModel, WizardParams>(
            WizardParams);
    }
}
...
private async Task NavigateBasedOnRole()
{
    if (User.IsAdmin)
    {
        await Navigator.NavigateToAsync<AdminDashboardViewModel>();
    }
    else if (User.IsManager)
    {
        await Navigator.NavigateToAsync<ManagerDashboardViewModel>();
    }
    else
    {
        await Navigator.NavigateToAsync<UserDashboardViewModel>();
    }
}
```
...
private async Task NavigateToProductReview(string productId, string reviewId)
{
    ...
    await Navigator.NavigateToAsync<ReviewDetailViewModel, ReviewDetailParams>(
...
1. Replace `INavigator` with `INavigator` in ViewModels
