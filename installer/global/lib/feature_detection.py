...
- taskwright works standalone (task execution + quality gates)
    ...
    def check_compatibility(self) -> Dict[str, any]:
        ...
        if self.is_require_kit_installed() and not self.is_taskwright_installed():
            result["warnings"].append(
                "require-kit installed without taskwright - requirements management available, "
                "install taskwright for full integration (task execution)"
            )

        if self.is_taskwright_installed() and not self.is_require_kit_installed():
            result["warnings"].append(
    ...
    def get_feature_status_message(self) -> str:
        ...
        if len(packages) == 1:
            if packages[0] == "taskwright":
                return "taskwright installed (task execution only, install require-kit for requirements management)"
            elif packages[0] == "require-kit":
                return "require-kit installed (requirements management only, install taskwright for task execution)"

        if "taskwright" in packages and "require-kit" in packages:
            return "Full Agentecflow (taskwright + require-kit with bidirectional integration)"
